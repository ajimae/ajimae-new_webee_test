...
export class EventsService {
  ...
  async getFutureEventWithWorkshops(): Promise<any> {
    ...
    const events = qb.where("event.id IN " + qb.subQuery.prototype.select("event.id").from(Event, "events").where("event.createdAt > :currentDate" + qb.subQuery().select("workshop").from(Workshop, "workshop").where("workshop.start > :currentDate").limit(1).getQuery()).getQuery())
